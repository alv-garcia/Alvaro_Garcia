...
mudancas %>% 
  filter(Entity == "World") %>% 
  select(Entity,Date,Temperature.anomaly,SeaLevel = "Average")-> mud_final

mud_final2$yearMonth <- floor_date(mud_final2$Time,unit = "month")

mud_final %>% 
  mutate(Time = ymd(Date))->mud_final2


mud_agre <- mud_final2 %>% 
  group_by(yearMonth) %>% 
  summarize(Temperature.anomaly = mean(Temperature.anomaly))


ggplot(mud_agre )+
  geom_line(mapping = aes(x = yearMonth, y= Temperature.anomaly))
